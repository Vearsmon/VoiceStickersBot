/*namespace VoiceStickersBot.Core.Commands.SwitchKeyboard;

public class SwitchKeyboardCommandObsolete : ICommand
{
    public PageChangeDirection PageChangeDirection { get; }
    public int? PageFrom { get; }
    public int StickersOnPage { get; }
    public string KeyboardCapture { get; }
    public Type CommandType => typeof(SwitchKeyboardCommandObsolete);
    public RequestContext RequestContext { get; }


    public SwitchKeyboardCommandObsolete(RequestContext requestContext, int? pageFrom, string commandText, int stickersOnPage, string keyboardCapture="")
    {
        var pageDirection = commandText.Split(':').First();
        
        if (pageDirection == "pageleft")
            PageChangeDirection = PageChangeDirection.Decrease;
        else if (pageDirection == "pageright")
            PageChangeDirection = PageChangeDirection.Increase;

        RequestContext = requestContext;
        PageFrom = pageFrom;
        StickersOnPage = stickersOnPage;
        KeyboardCapture = keyboardCapture;
    }
}*/